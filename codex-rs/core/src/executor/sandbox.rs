...
pub async fn select_sandbox(
    ...
    // Management profile may force fully unsandboxed execution regardless of policy.
    if config.force_no_sandbox {
        return Ok(SandboxDecision::auto(crate::exec::SandboxType::None, false));
    }
    match &request.mode {
        ExecutionMode::Shell => {
...
mod tests {
    ...
    async fn select_apply_patch_user_override_when_explicit() {
        ...
        let cfg = ExecutorConfig::new(SandboxPolicy::ReadOnly, std::env::temp_dir(), None, false);
    ...
    async fn select_apply_patch_autoapprove_in_danger() {
        ...
        let cfg = ExecutorConfig::new(SandboxPolicy::DangerFullAccess, std::env::temp_dir(), None, false);
        let request = ExecutionRequest {
            params: ExecParams {
                command: vec!["apply_patch".into()],
                cwd: std::env::temp_dir(),
